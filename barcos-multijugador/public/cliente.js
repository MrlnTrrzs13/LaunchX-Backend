document.addEventListener('DOMContentLoaded', () => {
    ...
    const corrier = document.querySelector('.corrier-container');
    ...
    function startMultiplayer() {
        ...
        socket.on('player-connection', num => {
            console.log(`Jugador ${num}se fué`);
            playersConnectedOrDisconnected(num);
        });
        ...
        socket.on('check-player', players => {
            players.forEach((p, i) => {
                if(p.connected) playersConnectedOrDisconnected(i);
                if(p.ready) {
                    playerReady(i);
        ...
        socket.on('timeout', () => {
            infoDisplay.innerHTML = '¿Sigues ahi? Se te acabo el tiempo';
        });
        ...
        cpuSquares.forEach(squre => {
            squre.addEventListener('click', () => {
                if(currentPlayer === 'user' && ready && enemyReady){
                    shortsFired = squre.dataser.id;
                    socket.emit('fire', shortsFired)
                }
            });
        });
    ...
    function createBoard(grid, squeres) {
    ...
    function generate (ship) {
        let randomDirection = Math.floor(Math.random() * 
        ship.directions.length);
        ...
        const isTaken = current.some(index => cpuSquares[randomStart + index].
        ...
        if (!isTaken && !isAtRightEdge && !isAtLeftEdge)
            current.forEach(index => computerSquares[randomStart +
            index].classList.add('taken', ship.name))
        else generate(ship)
    };
